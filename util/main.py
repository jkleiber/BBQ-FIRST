...
async def load_banners(banner_mode: str, year: int):
    if banner_mode == "since_current_year":
        result = await data_loader.load_banners_since_current_year()
    elif banner_mode == "since":
        result = await data_loader.load_banners_since(year)
    elif banner_mode == "year":
        result = await data_loader.load_year_banners(year)
    elif banner_mode == "full":
        result = await data_loader.load_all_banners()

    print(result)

async def load_teams(team_mode):
    if team_mode == "info":
        report = await data_loader.load_team_info()
    elif team_mode == "data":
        report = await data_loader.load_team_data()
    elif team_mode == "full":
        info_report = await data_loader.load_team_info()
        data_report = await data_loader.load_team_data()
        report = {
            'info': info_report,
            'data': data_report
        }
...
async def load_events(event_mode: str, year: int, update_event_data: bool):
    if event_mode == "since_current_year":
        report = await data_loader.load_events_since_current_year(update_event_data)
    elif event_mode == "since":
        report = await data_loader.load_events_since(year, update_event_data)
    elif event_mode == "year":
        report = await data_loader.load_year_events(year, update_event_data)
    elif event_mode == "full":
        report = await data_loader.load_all_events(update_event_data)
...
if __name__ == "__main__":
    ...
    if args.mode == "banner":
        asyncio.run(load_banners(args.banner_mode, args.year))
    elif args.mode == "team":
        asyncio.run(load_teams(args.team_mode))
    elif args.mode == "event":
        asyncio.run(load_events(args.event_mode, args.year, not args.info_only))
